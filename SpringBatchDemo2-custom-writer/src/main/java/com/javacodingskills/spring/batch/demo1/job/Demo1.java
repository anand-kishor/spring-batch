...
public class Demo1 {
	...
	public Step stepDemo1() {
		...
		return this.stepBuilserFactory.get("step1").<EmployeeDTO,Employee>chunk(5).reader(employeeReader()).processor(new EmployeeProcessor()).writer(customWriter).build();
	}
	@Bean
	...
	/*
	 * @Bean public JdbcBatchItemWriter<Employee> employeeWriter() { // TODO
	 * Auto-generated method stub JdbcBatchItemWriter<Employee> writer=new
	 * JdbcBatchItemWriter<Employee>(); writer.setDataSource(dataSource); writer.
	...
	public FlatFileItemReader<EmployeeDTO> employeeReader() {
		...
		reader.setLineMapper(new DefaultLineMapper<EmployeeDTO>() {{setLineTokenizer(new DelimitedLineTokenizer() {{setNames("employeeId","firstName","lastName","email","age");
		}});
